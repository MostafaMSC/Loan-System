...
const fetchPreviousPrivatePayments = async () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
      ...cleanFormData,
      // Send the payment arrays instead of the string values
          ...
          <div className="card-body">
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div className="row">
                ...
                {renderInput('الحالة', 'Status', 'text', false, { readOnly: true })}
...
<h6 
  className={`text-info  pb-2 ${showPaymentsCash ? 'text-danger' : 'text-info'}`}
...
<h6
  className={`text-info pb-2 ${showPaymentsPrivate ? 'text-danger' : 'text-info'}`}
  style={{ cursor: 'pointer' }}
  ...
  {showPaymentsPrivate ? 'إخفاء الدفعات الخاصة'  : 'الاطلاع على الدفعات الخاصة السابقة'}
                ...
                <div className="col-12 mb-4">
                  ...
                  <div className="alert alert-info">
                    ...
                    <span className="text-success fs-5">{totalCashPayments.toLocaleString()} دينار</span>
                    {cashPayments.filter(p => p.Amount).length > 0 && (
